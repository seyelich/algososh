...
export const reverse = async (setLetters: Dispatch<SetStateAction<TArr<string>>>, setIsLoading: Dispatch<SetStateAction<boolean>>, word: string) => {
  setIsLoading(true);
  const arr = word.split('').map((val => ({ val, color: ElementStates.Default })));
...
export const StringComponent: React.FC = () => {
  ...
  const onLettersSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    reverse(setLetters, setIsLoading, values.word);
    setValues({word: ''});
  }
